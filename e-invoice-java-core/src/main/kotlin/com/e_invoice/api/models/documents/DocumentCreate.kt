...
private constructor(
    ...
    private constructor(
        ...
        private val priceBaseQuantity: JsonField<PriceBaseQuantity>,
        ...
        private constructor(
            ...
            @ExcludeMissing
            priceBaseQuantity: JsonField<PriceBaseQuantity> = JsonMissing.of(),
         ...
         *   the server responded with an unexpected value).
         */
        fun priceBaseQuantity(): Optional<PriceBaseQuantity> =
        ...
        @JsonProperty("price_base_quantity")
        @ExcludeMissing
        fun _priceBaseQuantity(): JsonField<PriceBaseQuantity> = priceBaseQuantity
        ...
        class Builder internal constructor() {
            ...
            private var priceBaseQuantity: JsonField<PriceBaseQuantity> = JsonMissing.of()
             ...
             * applies. Defaults to 1. Must be rounded to maximum 4 decimals
             */
            fun priceBaseQuantity(priceBaseQuantity: PriceBaseQuantity?) =
             ...
             */
            fun priceBaseQuantity(priceBaseQuantity: Optional<PriceBaseQuantity>) =
                priceBaseQuantity(priceBaseQuantity.getOrNull())

            /**
             * Sets [Builder.priceBaseQuantity] to an arbitrary JSON value.
             *
             * You should usually call [Builder.priceBaseQuantity] with a well-typed
             * [PriceBaseQuantity] value instead. This method is primarily for setting the field to
             * an undocumented or not yet supported value.
             */
            fun priceBaseQuantity(priceBaseQuantity: JsonField<PriceBaseQuantity>) = apply {
                this.priceBaseQuantity = priceBaseQuantity
            }

            /** Alias for calling [priceBaseQuantity] with `PriceBaseQuantity.ofNumber(number)`. */
            fun priceBaseQuantity(number: Double) =
                priceBaseQuantity(PriceBaseQuantity.ofNumber(number))

            /** Alias for calling [priceBaseQuantity] with `PriceBaseQuantity.ofString(string)`. */
            fun priceBaseQuantity(string: String) =
                priceBaseQuantity(PriceBaseQuantity.ofString(string))
        ...
        fun validate(): Item = apply {
            ...
            date()
            description()
            priceBaseQuantity().ifPresent { it.validate() }
            productCode()
        ...
        internal fun validity(): Int =
            (allowances.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) +
                ...
                (priceBaseQuantity.asKnown().getOrNull()?.validity() ?: 0) +
         ...
         * applies. Defaults to 1. Must be rounded to maximum 4 decimals
         */
        @JsonDeserialize(using = PriceBaseQuantity.Deserializer::class)
        @JsonSerialize(using = PriceBaseQuantity.Serializer::class)
        class PriceBaseQuantity
        private constructor(
            private val number: Double? = null,
            private val string: String? = null,
            private val _json: JsonValue? = null,
            ...
            fun validate(): PriceBaseQuantity = apply {
                if (validated) {
                    return@apply
                }
            ...
            override fun equals(other: Any?): Boolean {
                ...
                return other is PriceBaseQuantity &&
                    number == other.number &&
                    string == other.string
            }
            ...
            override fun toString(): String =
                when {
                    number != null -> "PriceBaseQuantity{number=$number}"
                    string != null -> "PriceBaseQuantity{string=$string}"
                    _json != null -> "PriceBaseQuantity{_unknown=$_json}"
                    else -> throw IllegalStateException("Invalid PriceBaseQuantity")
                }

            companion object {

                @JvmStatic fun ofNumber(number: Double) = PriceBaseQuantity(number = number)

                @JvmStatic fun ofString(string: String) = PriceBaseQuantity(string = string)
            }

            /**
             * An interface that defines how to map each variant of [PriceBaseQuantity] to a value
             * of type [T].
             */
            interface Visitor<out T> {
                ...
                /**
                 * Maps an unknown variant of [PriceBaseQuantity] to a value of type [T].
                 *
                 * An instance of [PriceBaseQuantity] can contain an unknown variant if it was
                 * deserialized from data that doesn't match any known variant. For example, if the
                 * SDK is on an older version than the API, then the API may respond with new
                 * variants that the SDK is unaware of.
                 *
                 ...
                 */
                fun unknown(json: JsonValue?): T {
                    throw EInvoiceInvalidDataException("Unknown PriceBaseQuantity: $json")
                }
            }

            internal class Deserializer :
                BaseDeserializer<PriceBaseQuantity>(PriceBaseQuantity::class) {

                override fun ObjectCodec.deserialize(node: JsonNode): PriceBaseQuantity {
                    val json = JsonValue.fromJsonNode(node)

                    val bestMatches =
                        sequenceOf(
                                tryDeserialize(node, jacksonTypeRef<Double>())?.let {
                                    PriceBaseQuantity(number = it, _json = json)
                                },
                                tryDeserialize(node, jacksonTypeRef<String>())?.let {
                                    PriceBaseQuantity(string = it, _json = json)
                                },
                            )
                            .filterNotNull()
                            .allMaxBy { it.validity() }
                    ...
                    return when (bestMatches.size) {
                        // This can happen if what we're deserializing is completely incompatible
                        // with all the possible variants (e.g. deserializing from object).
                        0 -> PriceBaseQuantity(_json = json)
                        1 -> bestMatches.single()
                        // If there's more than one match with the highest validity, then use the
                        // first completely valid match, or simply the first match if none are
                        // completely valid.
            ...
            internal class Serializer :
                BaseSerializer<PriceBaseQuantity>(PriceBaseQuantity::class) {

                override fun serialize(
                    value: PriceBaseQuantity,
                    generator: JsonGenerator,
                    provider: SerializerProvider,
                ) {
                    when {
