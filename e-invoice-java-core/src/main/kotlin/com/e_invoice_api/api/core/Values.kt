package com.e_invoice_api.api.core
...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { EInvoiceInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw EInvoiceInvalidDataException("`$name` is not set")
            is JsonNull -> throw EInvoiceInvalidDataException("`$name` is null")
            else -> throw EInvoiceInvalidDataException("`$name` is invalid, received $this")
        }
